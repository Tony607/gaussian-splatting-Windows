...
，如果是老的cuda 11.6，需要更新，命令行输入`nvcc --version` 检测当前CUDA版本。

2. 下载解压 **COLMAP**（点云+相机姿态生成）--[下载直达地址](https://github.com/colmap/colmap/releases/download/3.8/COLMAP-3.8-windows-cuda.zip)，下载后解压到当前git库目录下`tools`子文件夹（手动创建一个`tools`文件夹），将`COLMAP-3.8-windows-cuda`目录添加到`PATH`系统路径，比如`E:\GitHub\gaussian-splatting\tools\COLMAP-3.8-windows-cuda`
...
4. COLMAP可视化点云（可选步骤）：双击`COLMAP.bat`运行, 如`E:\GitHub\gaussian-splatting\tools\COLMAP-3.8-windows-cuda\COLMAP.bat`，选择`data\sparse\0`，如`E:\GitHub\gaussian-splatting\data\sparse\0`
5. 训练模型（约5~15分钟）：`python train.py -s data -m data/output`（在模型训练到7000步即可进入下一步可视化）
6. 实时可视化渲染：`.\tools\viewers\bin\SIBR_gaussianViewer_app -m data/output`
